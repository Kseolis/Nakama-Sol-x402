...
Архитектурные решения пишутся до кода. Каждый ADR проходил architect-агента и security-агента до мёржа.

| ADR | Тема |
|---|---|
| [001](docs/architecture/adr-001-account-model.md) | Account layout, reserved-bytes pattern, no-`realloc` law |
| [002](docs/architecture/adr-002-escrow-ownership.md) | Один escrow, vault как PDA-owned TokenAccount, subscribe + cancel flow |
| [003](docs/architecture/adr-003-subscription-fsm.md) | Subscription FSM (Active / GracePeriod / Cancelled / Exhausted) |
| [004](docs/architecture/adr-004-charge-invariants.md) | Streaming-математика, permissionless charge, переход в grace |
| [005](docs/architecture/adr-005-variable-rate.md) | Variable-rate charging, отложен из MVP |
| [006](docs/architecture/adr-006-pause-resume.md) | Pause/Resume с time-frozen continuity, satellite PDA |
| [007](docs/architecture/adr-007-topup-grace-integration.md) | Top-up из Active и GracePeriod, grace recovery |
| [008](docs/architecture/adr-008-resubscribe-pattern.md) | Re-subscribe после Cancelled/Exhausted через cleanup tombstone |
| [009](docs/architecture/adr-009-cancel-by-merchant.md) | Полиморфный cancel — подписчик или мерчант как signer |
| [013](docs/architecture/adr-013-cancel-decomposition.md) | Cancel разделён на settle+refund и cleanup (rent reclaim) |
| [014](docs/architecture/adr-014-create-plan-invariants.md) | Plan PDA, USDC mint whitelist, merchant signer |
| [x402-001](docs/architecture/adr-x402-001-pay-session-layer.md) | PaySession satellite, facilitator delegation, композабильность с charge |
| [roadmap](docs/architecture/adr-roadmap.md) | Последовательность ADR и forward-compat заметки |
...
AI dev tooling: Claude Code (Opus + Sonnet) с командой из 7 субагентов (architect, reviewer, anchor-engineer, off-chain Rust, SDK, test-engineer, security-auditor) и ADR pipeline, который пропускает каждое изменение через architect-reviewer + security-auditor до мёржа. Драфты ADR и review-репорты лежат в `docs/architecture/`.
...
Детальный статус каждого пункта: [`docs/architecture/future-work.md`](docs/architecture/future-work.md).
